...
13. [GitHub のアカウントとプラン](#13-github-のアカウントとプラン)
14. [GitHub の請求](#14-github-の請求)
15. [標準の組み込み自動化（GitHub Actions）](#15-標準の組み込み自動化github-actions)
...
## 13. GitHub のアカウントとプラン

> **アカウントの種類**と**プラン**は別物です。まず「どの種類のアカウントか」、次に「どのプランか」を分けて理解します。

### アカウントの種類（3種類）

| 種類 | 概要 | サインイン |
|---|---|---|
| **個人用（ユーザー）** | GitHub上の個人のID（ユーザー名・プロフィール）。リポジトリ等を所有し、issue作成やPRレビュー等のアクションは個人アカウントに帰属 | できる |
| **組織（Organization）** | 多数のユーザーが共同作業する共有アカウント。権限は階層的（ロール付与）。**所有者・セキュリティマネージャー**のみ高度な管理が可能 | できない（各自が個人アカウントでサインインして操作） |
| **Enterprise** | 複数の組織のポリシー・請求を一元管理。組織間のInner Sourcingを実現。ハンドルを持つ | — |

> 個人アカウントは複数の組織のメンバーになれます。組織のリソースに対する操作も、その人の個人アカウントに帰属します。

### プラン

| プラン | 対象 | 主な内容（抜粋） |
|---|---|---|
| **GitHub Free** | 個人 / 組織 | 無制限のpublic/privateリポジトリ・コラボレーター、Dependabotアラート、2FA強制、Packages 500MB、Actions 2,000分/月。組織向けはチームのアクセス制御も |
| **GitHub Pro** | 個人 | Freeに加え、メールサポート、Actions 3,000分/月、privateの高度ツール（必須/複数PRレビュー、保護ブランチ、コード所有者、Pages/Wiki、分析グラフ 等） |
| **GitHub Team** | 組織 | 組織Freeを超える協業・セキュリティ・ワークフロー機能（ドラフトPR、Teamレビュー担当者、保護ブランチ、スケジュールリマインダー、Codespaces有効化 等） |
| **GitHub Enterprise** | 大規模組織 | Teamの全機能＋専用サポート、外部IdPによる一元認証、GitHub Connect、GitHub Advanced Security購入オプション 等 |

### GitHub Enterprise の2つの形態

| | GitHub Enterprise Server (GHES) | GitHub Enterprise Cloud |
|---|---|---|
| 提供形態 | **セルフホスト**（インフラを自社で完全管理） | GitHub.com 上のクラウド |
| 使用量 | — | Actions 50,000分/月・Packages 50GB と大幅増、SLA 99.9% |
| 管理 | — | 複数組織のポリシー/請求の一元管理、Enterprise Managed Users(EMU) |

> **Note**: GitHub Copilot は**既定ではどのプランにも含まれません**。有効なプランに **Copilot Business / Copilot Enterprise** を追加できます。

### Enterprise Managed Users (EMU)
IDプロバイダーを使って組織がユーザーIDを制御でき、集中アクセス管理とセキュリティ強化が可能になる仕組みです。

---

## 14. GitHub の請求

GitHub は**アカウントの種類（個人・組織・企業）に基づいて課金**されます。個人とほとんどの組織は個別請求、Enterpriseは複数組織の請求集約やコストセンター振り分けが可能です。

### 請求の2本立て

| 区分 | 内容 |
|---|---|
| **サブスクリプション** | プラン（Pro/Team等）や Copilot・Marketplaceアプリなどの**固定月額** |
| **使用量ベース** | GitHub Actions（実行分＋成果物ストレージ）や Packages など、**使った分だけ**増減 |

> **Note**: 多くのプランに一定の無料枠があります（例：GitHub Pro は Actions 3,000分/月が無料）。**使用制限**を設定して超過を管理できます。

### GitHub Actions の価格
- **パブリックリポジトリ**：GitHubホストランナーで**無料・無制限**
- **プライベートリポジトリ**：プランごとに無料枠（例：個人Freeは2,000分/月）。超過分は使用制限で管理
- 特にアクティブなprivateリポジトリは、想定外コスト回避のため**使用状況の監視**が重要

### ライセンスの使用量ベース課金（従量制）
| 仕組み | 内容 |
|---|---|
| 月単位の請求 | 実際に使用した Enterprise / Advanced Security ライセンス数に応じて毎月請求 |
| 日割り | 月途中から使い始めた利用分は日割り |
| 動的調整 | 月途中で使用停止すると翌月請求に反映（アクティブ分のみ支払い） |

> 事前のライセンス一括購入が不要になり、柔軟性とコスト効率が向上。

### 請求プラットフォームの機能
- **きめ細かな支出管理**：Actions/Packages等にサービス別の使用制限を設定し超過防止
- **詳細な使用状況分析**：消費パターンを把握しリソース割り当てを最適化
- **自動レポート**：使用状況レポートの自動化で内部チャージバックを効率化
- **GitHub Codespaces**：プランに応じてストレージ・コンピュート時間で追加課金
- **課金アラート**：使用制限が近づくと UI・メールで通知

---

## 15. 標準の組み込み自動化（GitHub Actions）

GitHub には、**標準で用意された自動化（スターターワークフロー）**があり、ボタン操作だけで CI/CD やタスク自動化を始められます。
ここでは「標準の組み込み自動化を行う手順」と、このリポジトリに置いた**例題**を説明します。

### 組み込み自動化を行う手順（GitHub UI）

1. リポジトリ上部の **[Actions]** タブを開く
2. **「New workflow」** をクリック（GitHubが用途別の**標準テンプレート＝スターターワークフロー**を一覧表示）
3. 使いたいテンプレートの **「Configure」** を選ぶ
   - 例：シンプルな入門用は **「Simple workflow」** や **「GitHub Actions Demo」**
   - CI なら言語別（Node.js / Python / Java など）のテンプレート
4. 内容を確認し、必要なら編集して **「Commit changes」**
   - ファイルは `.github/workflows/` 配下に保存される（これがワークフロー定義）
5. **`on:` で指定したトリガー**（push / pull_request / schedule / 手動 など）で**自動実行**される
6. 実行結果は **[Actions] タブ**でログとともに確認できる

### このリポジトリの例題

標準デモとほぼ同じ内容のワークフローを用意しました。
**[`.github/workflows/github-actions-demo.yml`](.github/workflows/github-actions-demo.yml)**

```yaml
name: GitHub Actions Demo
on: [push]              # ← push されると自動実行（トリガー）
jobs:
  Explore-GitHub-Actions:
    runs-on: ubuntu-latest   # ← GitHubホストランナー上で実行
    steps:
      - run: echo "🎉 ${{ github.event_name }} で自動起動されました。"
      - uses: actions/checkout@v4   # ← 標準アクション（コードを取得）
      - run: ls ${{ github.workspace }}
```

#### 動かし方
- このリポジトリへ **push すると自動で実行**されます
- 実行を見るには **[Actions] タブ → 「GitHub Actions Demo」** を開き、ジョブのログを確認

### 用語ミニ解説
| 用語 | 意味 |
|---|---|
| ワークフロー | 自動化の定義ファイル（`.github/workflows/*.yml`） |
| トリガー (`on:`) | 自動実行のきっかけ（push / pull_request / schedule / workflow_dispatch 等） |
| ジョブ / ステップ | 実行単位。ジョブの中に複数のステップ（コマンドやアクション）を並べる |
| ランナー (`runs-on`) | ワークフローを実行するマシン（GitHubホスト or セルフホスト） |
| アクション (`uses:`) | 再利用可能な部品（例：`actions/checkout` でコード取得） |

> このリポジトリには他に、コードスキャンの [`codeql.yml`](.github/workflows/codeql.yml)（第9章）もあります。どちらも「組み込み自動化」の一種です。
